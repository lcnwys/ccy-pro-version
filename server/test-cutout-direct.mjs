...
// 步骤 2: 调用抠图 API (使用 referenceImageId - 驼峰)
async function testCutoutCamelCase(fileId) {
  console.log('【步骤 2】调用 cut-out-portrait (驼峰命名 referenceImageId)...');
  console.log(`  Payload: ${JSON.stringify({ schema: 'basic', referenceImageId: fileId })}`);
...
// 步骤 3: 调用抠图 API (使用 reference_image_id - 下划线)
async function testCutoutSnakeCase(fileId) {
  console.log('\n【步骤 3】调用 cut-out-portrait (下划线命名 reference_image_id)...');
  console.log(`  Payload: ${JSON.stringify({ schema: 'basic', reference_image_id: fileId })}`);
...
// 步骤 4: 查询任务结果
async function queryResult(taskId, testName) {
  if (!taskId) return;

  console.log(`\n【步骤 4】查询任务结果 (${testName})...`);
